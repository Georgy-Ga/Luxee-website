...
const aiAutoResponseService = {
		...
		try {
			...
			const timeoutId = setTimeout(async () => {
				...
				try {
					...
					console.log(
						`[Pending] 📊 History: ${history.messages.length} messages, last from: ${history.lastMessage.author}`,
					);
					...
					console.log(
						`[Pending] 📝 Message type: ${history.lastMessage.messageType}`,
					);
					...
					console.error(
						`[Pending] ❌ Error executing response for ${chatId}:`,
						error,
					);
					pendingResponses.delete(chatId);
				}
	...
	_cancelAllPendingForAccount: accountId => {
		...
		for (const [chatId, pending] of pendingResponses.entries()) {
			if (pending.accountId === accountId) {
				...
				cancelledCount++;
				console.log(
					`[Pending] ❌ Cancelled pending response for chat ${chatId}`,
				);
			}
		}
		...
		if (cancelledCount > 0) {
			console.log(
				`[Pending] 🚫 Cancelled ${cancelledCount} pending responses for account ${accountId}`,
			);
		}
	},
	 ...
	 * @private
	 */
	_processProfileWithRetries: async ({
		accountId,
		userId,
		page,
		profile,
		...
		try {
			for (let attempt = 1; attempt <= maxAttempts; attempt++) {
				console.log(
					`[AI Auto] 🔄 Attempt ${attempt}/${maxAttempts} to find unanswered chats on ${profile.username}...`,
				);
				...
				if (!profileData) {
					console.log(
						`[AI Auto] ⚠️ Profile ${profile.uid} not found in modelsChat`,
					);
					return;
				}

				console.log(
					`[AI Auto] Profile ${profile.username} has ${profileData.allUids.length} UIDs (${profileData.hasOuter} outer)`,
				);
				...
				if (unansweredChats.length > 0) {
					console.log(
						`[AI Auto] ✅ Found ${unansweredChats.length} unanswered chats on ${profile.username} (attempt ${attempt})`,
					);
					...
					try {
						...
						if (isAlreadyAnswered) {
							console.log(
								`[AI Auto] Chat ${chat.chatId} already answered, skipping`,
							);
						...
						console.log(
							'  🕐 Message time:',
							new Date(chat.lastManMessage.createdAt).toLocaleString(),
						);
						...
						if (scheduled.scheduled) {
							console.log(
								`[AI Auto] ✅ Response scheduled for ${chat.memberUsername} in ${Math.round(randomDelay / 1000)} seconds`,
							);
							return { scheduled: true }; // ← ВОЗВРАТ: успешно запланировано
						} else {
							console.log(
								`[AI Auto] ⚠️  Failed to schedule: ${scheduled.reason}`,
							);
							return { scheduled: false, reason: scheduled.reason };
						}
					} catch (error) {
						console.error(
							`[AI Auto] Error processing chat ${chat.chatId}:`,
							error.message,
						);
			...
			console.log(
				`[AI Auto] ❌ No unanswered chats found on ${profile.username} after ${maxAttempts} attempts`,
			);
		} catch (error) {
			console.error(
				`[AI Auto] Error in _processProfileWithRetries for ${profile.username}:`,
				error,
			);
		}
	},
		...
		try {
			...
			for (let i = 0; i < unansweredChats.length; i++) {
				...
				try {
					...
					if (history.error) {
						console.log(
							`[AI Auto] ⚠️  Could not get chat history: ${history.error}, using fallback...`,
						);
					...
					console.log(
						`[AI Auto] 📝 Message type: ${history.lastMessage.messageType}`,
					);
			...
			console.error(
				`[AI Auto] Error processing profile ${profile.uid}:`,
				error,
			);
		}
	},
