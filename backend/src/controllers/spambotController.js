...
class SpambotController {
	...
	async checkAccountAvailability(req, res) {
		try {
			...
			const availability = await spambotService.checkAccountAvailability(accountId);
	...
	async createDistribution(req, res) {
		try {
			...
			console.log('[Spambot Controller] config:', JSON.stringify(config, null, 2));
			...
			const requiredFields = ['profileUid', 'profileName', 'distributionType', 'limit', 'filterUpdateLimit'];
			for (const field of requiredFields) {
				if (!config[field]) {
					console.log(`[Spambot Controller] ERROR: config.${field} is missing`);
					console.log(`[Spambot Controller] config.${field} value:`, config[field]);
					return res.status(400).json({ 
						success: false,
			...
			if (config.distributionType === 'chat' && (!config.messages || config.messages.length === 0)) {
	...
	async getDistributions(req, res) {
		try {
			...
			console.error('[Spambot Controller] Error getting distributions:', error);
			res.status(500).json({ 
				success: false,
				message: error.message 
			});
	...
	async stopDistribution(req, res) {
		try {
			...
			const result = await spambotService.stopDistribution(id, userId);
