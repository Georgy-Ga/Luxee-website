...
from src.core.exceptions import UserBlockedError, CompleteDestributionError, LuxeeLoginError, StopDestributionError
from src.core.logger import logger
from src.core.luxee_site.luxee_requests import LuxeeRequests
from src.core.models import Profile, Client, Distribution, Message, MailMessage
from src.core.utils import compare_strings_ignore_symbols, save_selenium_error, sanitize_list, sanitize_str
...
class Luxee:
        ...
        while distribution.sent_messages_count < distribution.limit:
            ...
            try:
                ...
                if sent_messages_on_page == 0:
                    empty_pages_count += 1
                    logger.warning(f"Empty page {empty_pages_count}/{max_empty_pages}: no messages sent on this page")
                    if empty_pages_count >= max_empty_pages:
                        logger.warning(f"Reached {max_empty_pages} empty pages in a row. All available clients processed or skipped. Stopping distribution.")
                        break
                else:
                    empty_pages_count = 0  # Сбрасываем счётчик при успешной отправке
    ...
    def start_mail_distribution(self, distribution: Distribution):
        ...
        while distribution.sent_messages_count < distribution.limit:
            # Проверка флага остановки
            if distribution.stop_flag and distribution.stop_flag.is_set():
                logger.info("Stop requested by user, breaking mail distribution loop")
                break
            ...
            try:
                ...
                for client in clients:
                    # Проверка флага остановки внутри цикла клиентов
                    if distribution.stop_flag and distribution.stop_flag.is_set():
                        logger.info("Stop requested by user, breaking mail client loop")
                        raise CompleteDestributionError("Stopped by user")
                ...
                if sent_messages_on_page == 0:
                    empty_pages_count += 1
                    logger.warning(f"Empty mail page {empty_pages_count}/{max_empty_pages}: no mails sent on this page")
                    if empty_pages_count >= max_empty_pages:
                        logger.warning(f"Reached {max_empty_pages} empty pages in a row. All available clients processed or skipped. Stopping mail distribution.")
                        break
                else:
                    empty_pages_count = 0  # Сбрасываем счётчик при успешной отправке
